...
class IUImageDataset(data.Dataset):
    def __init__(self, img_path, root_path, clip_pretrained=True):
        ...
        for _path in tqdm(imgs_df["filename"]):
            ...
            _np_img = np.asarray(Image.open(Path(os.path.join(root_path, _path))).resize((input_resolution, input_resolution), resample=Image.BICUBIC)) # these images all have diff sizes
